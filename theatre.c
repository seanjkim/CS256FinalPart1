...
void purchase()
{
	...
	while(row > 15 || row < 0 || column > 30 || column < 0)
	{
		printf("Please input a valid row and column.\n");
		fflush(stdout);
		scanf("%i %i", &row, &column);
	}
	while(seats[row][column] != 'o')
	{
		printf("Please input an available row and column.\n");
		fflush(stdout);
		scanf("%i %i", &row, &column);
	}
	row -= 1;
